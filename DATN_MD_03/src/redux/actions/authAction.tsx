...
export const login = (credentials: {username: string, password: string}) => async (dispatch: any) => {
  ...
  if (!validatePassword(credentials.password)) {
    dispatch({ type: LOGIN_FAILURE, payload: 'Mật khẩu phải có ít nhất 8 ký tự, bao gồm chữ hoa, chữ thường, số và ký tự đặc biệt' });
    return;
  }
...
export const register = (data: {email: string, password: string, username: string, role: string, birthDate: string}) => async (dispatch: any) => {
  ...
  if (!validateUsername(data.username)) {
    dispatch({ type: REGISTER_FAILURE, payload: 'Username phải từ 3-20 ký tự' });
    return;
  }
  if (!validatePassword(data.password)) {
    dispatch({ type: REGISTER_FAILURE, payload: 'Mật khẩu phải có ít nhất 8 ký tự, bao gồm chữ hoa, chữ thường, số và ký tự đặc biệt' });
    return;
  }
  if (!data.role) {
...
export const forgotPassword = (email: string) => async (dispatch: any) => {
  ...
  try {
    ...
    const result = await res.json();
    dispatch({ type: FORGOT_PASSWORD_SUCCESS, payload: result.message || 'Đã gửi email đặt lại mật khẩu!' });
  } catch (error: any) {
