...
const CustomAlertModal: React.FC<CustomAlertModalProps> = ({
  ...
  const getTypeIcon = () => {
    if (title.toLowerCase().includes('hợp đồng')) return Icons.IconHopDong;
    if (title.toLowerCase().includes('hệ thống')) return Icons.IconHeThong;
    if (title.toLowerCase().includes('thanh toán')) return Icons.IconThanhToan;
    if (title.toLowerCase().includes('hỗ trợ')) return Icons.IconHoTro;
    return Icons.IconNotification;
  };

  // Lấy màu theo loại thông báo
  const getTypeColor = () => {
    if (title.toLowerCase().includes('hợp đồng')) return '#059669';
    if (title.toLowerCase().includes('hệ thống')) return '#2563EB';
    if (title.toLowerCase().includes('thanh toán')) return '#EA580C';
    if (title.toLowerCase().includes('hỗ trợ')) return '#9333EA';
    return Colors.gray;
  };
