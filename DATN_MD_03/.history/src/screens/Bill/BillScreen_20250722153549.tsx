...
const BillScreen = () => {
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            const checkAndLoadData = async () => {
                try {
                    ...
                    // Tất cả người dùng đều kiểm tra người ở cùng (nếu có token)
                    if (token) {
                        console.log('Checking roommate status because token exists');
                        
                        // Luôn gọi API kiểm tra người ở cùng nếu có token
                        try {
                            console.log('Calling checkUserIsCoTenant directly');
                            const result = await checkUserIsCoTenant(token);
                            console.log('checkUserIsCoTenant result:', JSON.stringify(result, null, 2));
                            ...
                            if (isMounted.current) {
                                ...
                                console.log('Loading invoices based on coTenant check result');
                                
                                if (isCoTenant) {
                                    // Lấy hóa đơn người ở cùng
                                    console.log('Dispatching fetchRoommateInvoices');
                            ...
                            console.error('Error during coTenant check:', error);
                            if (isMounted.current) {
                                setIsUserCoTenant(false);
                                
                                console.log('Dispatching fetchInvoices after error');
                                dispatch(fetchInvoices({
                                    token,
                                    page: 1,
                                    limit: 10,
                        ...
                        console.log('No token, cannot check coTenant status');
                    }
        ...
        }, [token, user, isLandlord, dispatch, selectedStatus])
    );
