...
const BillScreen = () => {
    ...
    useEffect(() => {
        ...
        if (localInvoices.length > 0) {
            const regularInvoices = localInvoices.filter(inv => !inv.isRoommate);
            const roommateInvoices = localInvoices.filter(inv => inv.isRoommate);
            
            console.log('=== BILL SCREEN INVOICES SUMMARY ===');
            console.log('Tổng số hóa đơn:', localInvoices.length);
            console.log('Số hóa đơn thường:', regularInvoices.length);
            console.log('Số hóa đơn người ở cùng:', roommateInvoices.length);
            console.log('User role:', user?.role);
            console.log('Is user co-tenant:', isUserCoTenant);
            console.log('=====================================');
        }
    }, [localInvoices, user?.role, isUserCoTenant]);
    ...
    const loadInvoices = useCallback((page = 1, shouldRefresh = false) => {
        if (token) {
            ...
            // Luôn gọi action để lấy danh sách hóa đơn thông thường
            ...
            // Nếu người dùng là người thuê và là người ở cùng, lấy thêm hóa đơn người ở cùng
            if (user?.role === 'nguoiThue' && isUserCoTenant && user?._id) {
    ...
    const checkUserInCoTenants = useCallback(async () => {
        ...
        try {
            // Gọi hàm kiểm tra từ billService với userId
            const result = await checkUserIsCoTenant(token, user._id);
            
            if (result.success) {
                console.log('User coTenant check result:', { 
                    userId: user._id, 
                    isCoTenant: result.isCoTenant,
                ...
                // Nếu người dùng là người ở cùng, hiển thị thông báo
                if (result.isCoTenant) {
                    console.log('User is a co-tenant, will fetch roommate invoices');
                } else {
                    console.log('User is not a co-tenant, will only see regular invoices');
                }
            }
        } catch (error) {
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            // Chạy tuần tự để đảm bảo trạng thái coTenant được xác định trước khi tải hóa đơn
            const initializeScreen = async () => {
                // Kiểm tra trạng thái coTenant trước
                await checkUserInCoTenants();
                
                // Sau đó tải dữ liệu hóa đơn
                if (token) {
        ...
        }, [token, checkUserInCoTenants, user?.role, user?._id])
    );
    ...
    // useEffect để reload invoices khi trạng thái coTenant thay đổi
    useEffect(() => {
        if (token && user?.role === 'nguoiThue') {
            console.log('isUserCoTenant state changed:', isUserCoTenant);
            // Reload invoices khi trạng thái coTenant thay đổi
            loadInvoices(1, false);
        }
    }, [isUserCoTenant]);
