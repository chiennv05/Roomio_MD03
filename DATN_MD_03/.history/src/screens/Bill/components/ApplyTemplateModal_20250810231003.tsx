...
const ApplyTemplateModal: React.FC<ApplyTemplateModalProps> = ({
    ...
    // custom alert
    const { visible: alertVisible, alertConfig, showError, showSuccess, hideAlert } = useCustomAlert();
    ...
    const handleApplyTemplate = async () => {
        if (!contract || !contract._id) {
            showError('Không tìm thấy thông tin hợp đồng', 'Lỗi');
            return;
        }

        if (!templateId) {
            showError('Không tìm thấy thông tin mẫu hóa đơn', 'Lỗi');
            return;
        }
        ...
        try {
            ...
            if (!response.success) {
                ...
                showError(
                    response.message || 'Có lỗi xảy ra khi áp dụng mẫu hóa đơn. Vui lòng thử lại sau.',
                    'Lỗi khi áp dụng mẫu'
                );
                return;
            }
            ...
            if (newInvoiceId) {
                ...
                showSuccess('Hóa đơn đã được tạo thành công từ mẫu', 'Thành công');
            }
        } catch (error: any) {
