...
const InvoiceCreationModal: React.FC<InvoiceCreationModalProps> = ({
    ...
    // Function to check if selected date is valid (not before contract start date and not more than 2 months in the future)
    const isDateValid = (date: Date) => {
        ...
        // Ngày được chọn phải từ ngày bắt đầu hợp đồng trở đi và không quá 2 tháng trong tương lai
        return selectedDateOnly >= contractStartDate && selectedDateOnly <= maxStartDate;
    };
    ...
    const handleDateSelection = (date: Date) => {
        if (!isDateValid(date)) {
            // Check if it's because it's before contract start date or too far in the future
            if (!contract) {
    ...
    const handleDueDateSelection = (date: Date) => {
        if (!isDateValid(date)) {
            ...
            if (selectedDateOnly < contractStartDate) {
                ...
                showError('Không thể chọn ngày hết hạn quá 2 tháng trong tương lai. Vui lòng chọn ngày gần hơn.');
            }
            return;
        }
    ...
    useEffect(() => {
        // Kiểm tra xem ngày hết hạn hiện tại có còn hợp lệ với ngày bắt đầu mới không
        if (!isDueDateValid(selectedDueDate, selectedDate)) {
            // Nếu không hợp lệ, tự động cập nhật ngày hết hạn
            const newDueDate = new Date(selectedDate);
            newDueDate.setDate(newDueDate.getDate() + 10);
            setSelectedDueDate(newDueDate);
        }
    ...
    const handleCreateInvoice = async () => {
        ...
        // Validation bổ sung cho ngày hết hạn
        if (!isDueDateValid(selectedDueDate, selectedDate)) {
            showError('Ngày hết hạn không hợp lệ. Vui lòng kiểm tra lại ngày hết hạn.');
            return;
        }

        // Validation bổ sung cho ngày bắt đầu
        if (!isDateValid(selectedDate)) {
            if (!contract) {
                showError('Không tìm thấy thông tin hợp đồng');
                return;
            ...
            const contractStartDate = new Date(contract.contractInfo.startDate);
            showError(`Ngày bắt đầu không hợp lệ. Ngày bắt đầu phải từ ${formatDate(contractStartDate)} trở đi.`);
            return;
        }
        ...
        try {
            ...
            // Hiển thị thông báo thành công dựa trên includeServices
            if (includeServices) {
                showSuccess('Đã tạo hóa đơn thành công và tự động thêm các dịch vụ từ hợp đồng');
            } else {
                ...
                <View style={styles.modalContent}>
                    ...
                    <View style={styles.formSection}>
                        ...
                        <DatePicker
                            ...
                            cancelText="Hủy"
                            minimumDate={contract ? new Date(contract.contractInfo.startDate) : new Date()}
                            maximumDate={(() => {
                        ...
                        <DatePicker
                            ...
                            cancelText="Hủy"
                            minimumDate={(() => {
                                // Ngày hết hạn phải sau ngày bắt đầu ít nhất 1 ngày
                                const minDueDate = new Date(selectedDate);
                                minDueDate.setDate(minDueDate.getDate() + 1);
                                return minDueDate;
                            })()}
                            maximumDate={(() => {
                                // Ngày hết hạn không được quá 2 tháng sau ngày bắt đầu
                                const maxDueDate = new Date(selectedDate);
                                maxDueDate.setDate(maxDueDate.getDate() + 60);
                                return maxDueDate;
                            })()}
                            onConfirm={(date) => {
                        ...
                        <View style={styles.referenceInfo}>
                            ...
                            {contract && (
                                <Text style={styles.contractDateInfo}>
                                    Ngày bắt đầu hợp đồng: {formatDate(new Date(contract.contractInfo.startDate))}
                                </Text>
                            )}
                        </View>
