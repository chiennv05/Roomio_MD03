...
const BillScreen = () => {
    ...
    // Sử dụng giá trị từ Redux store thay vì state nội bộ
    ...
    useEffect(() => {
        if (invoices.length > 0) {
            ...
            if (user?.role === 'nguoiThue' && isCoTenant) {
    ...
    }, [invoices, selectedStatus, selectedRoom, selectedTenant, sortOrder, isLandlord, user?.role, isCoTenant]);
    ...
    }, [dispatch, token, selectedStatus, user?.role, user?._id, isCoTenant]);

    // Kiểm tra xem người dùng có trong danh sách coTenants không
    // Sử dụng thunk action từ Redux thay vì gọi API trực tiếp
    const checkUserInCoTenants = useCallback(async () => {
        ...
        try {
            ...
            const result = await dispatch(checkIsCoTenant({ token })).unwrap();
    ...
    useFocusEffect(
        ...
        }, [token, loadInvoices, checkUserInCoTenants, user?.role, isCoTenant])
    );
    ...
    }, [user?.role, isCoTenant, openDropdown]);
    
    // Render các dropdown bộ lọc
    const renderFilterTabs = () => {
        // Kiểm tra nếu người dùng là người thuê hoặc người ở cùng
        const isTenantOrCoTenant = user?.role === 'nguoiThue' || isCoTenant;
