...
const BillScreen = () => {
    ...
    // State để lưu trữ ID người dùng từ API
    const [currentUserId, setCurrentUserId] = useState<string | null>(null);

    // Hàm lấy thông tin người dùng trực tiếp từ API
    ...
    const loadInvoices = useCallback(async (page = 1, shouldRefresh = false) => {
        if (token) {
            ...
            if (user?.role === 'nguoiThue' && isUserCoTenant) {
                // Lấy ID người dùng từ API nếu chưa có
                const userId = currentUserId || await fetchCurrentUser();
                
                if (userId) {
                    console.log('Fetching roommate invoices for tenant user:', userId);
                    ...
                    dispatch(fetchRoommateInvoices({
                        ...
                        limit: 10,
                        status: selectedStatus,
                        userId: userId, // Truyền ID người dùng từ API
                    }));
                }
            }
        } else {
    ...
    }, [dispatch, token, selectedStatus, user?.role, isUserCoTenant, currentUserId, fetchCurrentUser]);
