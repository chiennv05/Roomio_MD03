...
const BillScreen = () => {
    ...
    const checkUserInCoTenants = useCallback(async () => {
        ...
        try {
            ...
            const result = await dispatch(checkUserCoTenant({ token, userId: user._id }));
            
            if (result.meta.requestStatus === 'fulfilled') {
                console.log('User coTenant check result:', result.payload);
                
                // Cập nhật state
                setIsUserCoTenant(result.payload);
