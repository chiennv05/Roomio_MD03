...
import { InvoiceCard, ContractSelectionModal, InvoiceCreationModal, TimeRangeModal } from './components';
...
type FilterType = 'status' | 'room' | 'tenant' | 'sort' | 'time';
...
const BillScreen = () => {
    ...
    const [selectedTimeRange, setSelectedTimeRange] = useState<{ minDays: number; maxDays: number }>({ minDays: 0, maxDays: 30 });
    ...
    useEffect(() => {
        ...
        if (searchText.trim()) {
            ...
            console.log('Filtered by search:', searchText, '- remaining:', allInvoices.length, '(removed', countBefore - allInvoices.length, ')');
        }

        // Lọc theo khoảng thời gian
        if (selectedTimeRange.minDays > 0 || selectedTimeRange.maxDays < 30) {
            const countBefore = allInvoices.length;
            const now = new Date();
            allInvoices = allInvoices.filter(invoice => {
                ...
                return daysDiff >= selectedTimeRange.minDays && daysDiff <= selectedTimeRange.maxDays;
            });
            console.log('Filtered by time range:', selectedTimeRange, '- remaining:', allInvoices.length, '(removed', countBefore - allInvoices.length, ')');
        }
    ...
    }, [invoices, selectedStatus, selectedRoom, selectedTenant, sortOrder, searchText, isLandlord, user?.role, isUserCoTenant, selectedTimeRange]);
    ...
    }, [dispatch, token, selectedStatus, selectedTimeRange, isUserCoTenant]);
    ...
    const handleRefresh = useCallback(() => {
        ...
        // Reset bộ lọc thời gian về mặc định
        setSelectedTimeRange({ minDays: 0, maxDays: 30 });
    ...
    useEffect(() => {
        ...
        // Reset bộ lọc thời gian khi role thay đổi
        setSelectedTimeRange({ minDays: 0, maxDays: 30 });
                    ...
                    <View key={tab.id} style={styles.dropdownWrapper}>
                        <TouchableOpacity
                            ...
                            <Text style={styles.dropdownButtonText} numberOfLines={1} ellipsizeMode="tail">
                                ...
                                {tab.id === 'time' && (selectedTimeRange.minDays > 0 || selectedTimeRange.maxDays < 30) &&
                                    `: ${selectedTimeRange.minDays === 0 ? 
                                        `Trong ${selectedTimeRange.maxDays} ngày gần đây` : 
                                        selectedTimeRange.maxDays === 30 ? 
                                            `Từ ${selectedTimeRange.minDays} ngày trước trở đi` :
                                            `Từ ${selectedTimeRange.minDays} đến ${selectedTimeRange.maxDays} ngày trước`
                                    }`
                                }
                            </Text>
                            <Animated.Image
    ...
    const renderTimeFilter = () => {
        const getTimeRangeText = () => {
            ...
            if (selectedTimeRange.minDays === 0) {
                return `Trong ${selectedTimeRange.maxDays} ngày gần đây`;
            }
            if (selectedTimeRange.maxDays === 30) {
                return `Từ ${selectedTimeRange.minDays} ngày trước trở đi`;
            }
            return `Từ ${selectedTimeRange.minDays} đến ${selectedTimeRange.maxDays} ngày trước`;
        };
        ...
        return (
            <View style={styles.dropdownOptionsContainer}>
                <TouchableOpacity
                    style={[
                        styles.dropdownOption,
                        selectedTimeRange.minDays === 0 && selectedTimeRange.maxDays === 30 ? styles.dropdownOptionSelected : {},
                    ]}
                    onPress={() => {
                        setSelectedTimeRange({ minDays: 0, maxDays: 30 });
                        setOpenDropdown(null);
                    ...
                    <Text
                        style={[
                            styles.dropdownOptionText,
                            selectedTimeRange.minDays === 0 && selectedTimeRange.maxDays === 30 ? styles.dropdownOptionTextSelected : {},
                        ]}>
                        Tất cả thời gian
                    </Text>
                    {selectedTimeRange.minDays === 0 && selectedTimeRange.maxDays === 30 && (
                        <Image
                            source={require('../../assets/icons/icon_check.png')}
                            style={styles.checkIcon}
                        />
                ...
                <TouchableOpacity
                    style={[
                        styles.dropdownOption,
                        selectedTimeRange.minDays === 0 && selectedTimeRange.maxDays === 7 ? styles.dropdownOptionSelected : {},
                    ]}
                    onPress={() => {
                        setSelectedTimeRange({ minDays: 0, maxDays: 7 });
                        setOpenDropdown(null);
                    }}>
                    <Text
                        style={[
                            styles.dropdownOptionText,
                            selectedTimeRange.minDays === 0 && selectedTimeRange.maxDays === 7 ? styles.dropdownOptionTextSelected : {},
                        ]}>
                        Trong 7 ngày gần đây
                    </Text>
                    {selectedTimeRange.minDays === 0 && selectedTimeRange.maxDays === 7 && (
                        <Image
                            source={require('../../assets/icons/icon_check.png')}
                            style={styles.checkIcon}
                        />
                ...
                <TouchableOpacity
                    style={[
                        styles.dropdownOption,
                        selectedTimeRange.minDays === 0 && selectedTimeRange.maxDays === 90 ? styles.dropdownOptionSelected : {},
                    ]}
                    onPress={() => {
                        setSelectedTimeRange({ minDays: 0, maxDays: 90 });
                        setOpenDropdown(null);
                    }}>
                    <Text
                        style={[
                            styles.dropdownOptionText,
                            selectedTimeRange.minDays === 0 && selectedTimeRange.maxDays === 90 ? styles.dropdownOptionTextSelected : {},
                        ]}>
                        Trong 90 ngày gần đây
                    </Text>
                    {selectedTimeRange.minDays === 0 && selectedTimeRange.maxDays === 90 && (
                        <Image
                            source={require('../../assets/icons/icon_check.png')}
                            style={styles.checkIcon}
                        />
        ...
        <SafeAreaView style={styles.container}>
            ...
            <TimeRangeModal
                ...
                onConfirm={(minDays, maxDays) => {
                    setSelectedTimeRange({ minDays, maxDays });
                    setTimeRangeModalVisible(false);
                }}
                selectedMinDays={selectedTimeRange.minDays}
                selectedMaxDays={selectedTimeRange.maxDays}
            />
