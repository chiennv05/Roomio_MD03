...
const RoommateInvoiceDetailScreen = ({ route, navigation }: Props) => {
  ...
  const loadInvoiceDetails = useCallback(() => {
    if (token && invoiceId) {
      ...
      showError('Không thể tải thông tin hóa đơn', 'Lỗi');
    }
  }, [dispatch, token, invoiceId, showError]);
  ...
  useEffect(() => {
    if (markAsPaidSuccess) {
      showSuccess('Đã đánh dấu hóa đơn là đã thanh toán', 'Thành công');
      loadInvoiceDetails(); // Tải lại chi tiết hóa đơn
    }
    if (markAsPaidError) {
      showError(markAsPaidError, 'Lỗi');
    }
  }, [markAsPaidSuccess, markAsPaidError, loadInvoiceDetails, showSuccess, showError]);
