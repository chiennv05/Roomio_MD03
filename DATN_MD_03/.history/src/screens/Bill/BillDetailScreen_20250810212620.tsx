...
const BillDetailScreen = () => {
    ...
    // custom alert
    const { visible, alertConfig, showError, showSuccess, showConfirm, hideAlert } = useCustomAlert();
    ...
    useEffect(() => {
        if (confirmPaymentSuccess) {
            showSuccess('Đã xác nhận thanh toán hóa đơn thành công!', 'Thành công');
        ...
        if (confirmPaymentError) {
            showError(`Không thể xác nhận thanh toán hóa đơn: ${confirmPaymentError}`, 'Lỗi');
            dispatch(resetConfirmPaymentState());
        }
    ...
    useEffect(() => {
        if (markAsPaidSuccess) {
            showSuccess('Đã thanh toán hóa đơn thành công!', 'Thành công');
        ...
        if (markAsPaidError) {
            showError(`Không thể thanh toán hóa đơn: ${markAsPaidError}`, 'Lỗi');
            dispatch(resetMarkAsPaidState());
        }
    ...
    useEffect(() => {
        if (completeInvoiceSuccess) {
            showSuccess('Đã hoàn thành hóa đơn thành công!', 'Thành công');
            dispatch(resetCompleteInvoiceState());
        }

        if (completeInvoiceError) {
            showError(`Không thể hoàn thành hóa đơn: ${completeInvoiceError}`, 'Lỗi');
            dispatch(resetCompleteInvoiceState());
        }
    ...
    const handleConfirmPayment = () => {
        showConfirm(
            ...
            [
                ...
                { text: 'Xác nhận', onPress: () => { if (token && invoiceId) { dispatch(confirmPayment({ token, invoiceId })); } hideAlert(); } }
            ]
        );
    };
    ...
    const handleCompleteInvoice = () => {
        showConfirm(
            ...
            [
                { text: 'Hủy', onPress: hideAlert, style: 'cancel' },
                { text: 'Xác nhận', onPress: () => { if (token && invoiceId) { dispatch(completeInvoice({ token, invoiceId })); } hideAlert(); } }
            ]
        );
    };
