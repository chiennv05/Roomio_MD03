...
const InvoiceTemplatesScreen = () => {
    ...
    // Custom alert hook
    const { visible, alertConfig, showSuccess, showError, showConfirm, hideAlert } = useCustomAlert();
    ...
    useEffect(() => {
        if (deleteTemplateSuccess) {
            showSuccess('Đã xóa mẫu hóa đơn thành công', 'Thành công');
            dispatch(resetDeleteTemplateState());
        }
    }, [deleteTemplateSuccess]);
    ...
    useEffect(() => {
        if (deleteTemplateError) {
            showError(`Không thể xóa mẫu hóa đơn: ${deleteTemplateError}`, 'Lỗi');
            dispatch(resetDeleteTemplateState());
        }
    }, [deleteTemplateError]);
    ...
    const handleDeleteTemplate = (template: any) => {
        ...
        showConfirm(
            ...
            [
                ...
                { text: 'Xóa', onPress: () => { dispatch(deleteInvoiceTemplate({ token, templateId: template._id })); hideAlert(); }, style: 'destructive' }
            ]
        );
    };
