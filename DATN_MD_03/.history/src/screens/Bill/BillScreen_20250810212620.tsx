...
const BillScreen = () => {
    ...
    const handleInvoicePress = (invoice: Invoice) => {
        ...
        if (!invoiceId) {
            showError('Không thể mở chi tiết hóa đơn này', 'Lỗi');
            return;
        }
    ...
    const handleEditInvoice = (invoice: Invoice) => {
        ...
        if (!invoiceId) {
            showError('Không thể chỉnh sửa hóa đơn này', 'Lỗi');
            return;
        }
    ...
    const handleDeleteInvoice = (invoice: Invoice) => {
        ...
        const invoiceId = invoice._id || invoice.id;
        if (!invoiceId) {
            showError('Không thể xóa hóa đơn này', 'Lỗi');
            return;
        }
        ...
        showConfirm(
            ...
            () => { deleteInvoiceHandler(invoiceId); hideAlert(); },
            'Xác nhận xóa',
            [
                { text: 'Hủy', onPress: hideAlert, style: 'cancel' },
                { text: 'Xóa', onPress: () => { deleteInvoiceHandler(invoiceId); hideAlert(); }, style: 'destructive' },
            ],
        );
    };
    ...
    const deleteInvoiceHandler = async (invoiceId: string) => {
        if (!token) {
            showError('Bạn cần đăng nhập để thực hiện chức năng này', 'Lỗi');
            return;
        }
        ...
        try {
            ...
            if (response.success) {
                ...
                showSuccess(response.message || 'Đã xóa hóa đơn thành công', 'Thành công');
            ...
            showError(error.message || 'Có lỗi xảy ra khi xóa hóa đơn', 'Lỗi');
        }
    };
