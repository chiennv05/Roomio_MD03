...
const BillDetailScreen = () => {
    ...
    const renderInvoiceItems = () => {
        ...
        const renderItemDetails = (item: InvoiceItem) => {
            ...
            if (selectedInvoice?.dueDate) {
                rows.push({ label: 'Hạn thanh toán', value: formatDate(selectedInvoice.dueDate) });
            }
            rows.push({ label: 'Trạng thái thanh toán', value: getStatusText(selectedInvoice.status) });

            return (
                <View style={[styles.expandedDetailContainer, styles.expandedDetailConnected]}>
                ...
                <View style={styles.itemsGrid}>
                    {(() => {
                        ...
                        for (let i = 0; i < items.length; i += 2) {
                            ...
                            const hasExpanded = isLeftExpanded || isRightExpanded;
                            rows.push(
                                <View key={`row-${i}`} style={[styles.itemsRow, hasExpanded && styles.itemsRowExpanded]}>
                                    <TouchableOpacity style={[styles.itemCard, isLeftExpanded && styles.itemCardExpanded]} activeOpacity={0.85} onPress={() => toggleItemExpanded(leftKey)}>
                                    ...
                                    {right && (
                                        <TouchableOpacity style={[styles.itemCard, isRightExpanded && styles.itemCardExpanded]} activeOpacity={0.85} onPress={() => rightKey && toggleItemExpanded(rightKey)}>
                                            <View style={styles.itemHeaderRow}>
                                                <View style={styles.itemIconContainer}>
                            ...
                            if (isLeftExpanded) {
                                rows.push(<View key={`detail-${leftKey}`}>{renderItemDetails(left)}</View>);
                            } else if (isRightExpanded && right) {
                                rows.push(<View key={`detail-${rightKey}`}>{renderItemDetails(right)}</View>);
                            }
                        }
                        return rows;
                    })()}
