...
const BillScreen = () => {
    ...
    useEffect(() => {
        ...
        // Kiểm tra từng hóa đơn xem có phải người ở cùng không (chỉ để log)
        ...
        console.log('Current invoice breakdown:', {
            total: allInvoices.length,
            roommate: roommateInvoices.length,
            regular: regularInvoices.length,
            userRole: { isCoTenant: isUserCoTenant, isTenant: isUserTenant }
        });

        // Logic filter đã được xử lý trong service, không cần filter theo isRoommate nữa
    ...
    const handleLoadMore = () => {
        if (pagination.page < pagination.totalPages && !loading && token) {
            // Load more dữ liệu dựa trên vai trò người dùng
            if (user?.role === 'nguoiThue') {
                dispatch(fetchInvoicesByUserRole({
                    token,
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={{ paddingTop: statusBarHeight, alignItems: 'center', marginBottom: 30 }}>
                <UIHeader
                    title={
                        isUserCoTenant && isUserTenant 
                            ? 'Hóa đơn thuê trọ' 
                            : isUserCoTenant 
                                ? 'Hóa đơn người ở cùng' 
                                : 'Hóa đơn thu chi'
                    }
                    iconLeft={'back'}
