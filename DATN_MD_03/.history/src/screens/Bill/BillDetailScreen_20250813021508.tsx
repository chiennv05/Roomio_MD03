...
const BillDetailScreen = () => {
    ...
    // Debug: kiểm tra state expandedItems
    console.log('expandedItems state:', Array.from(expandedItems));
            ...
            <View style={styles.itemsContainer}>
                <View style={styles.itemsGrid}>
                    {items.map((item, index) => {
                        ...
                        // Debug: kiểm tra xem item nào đang expanded
                        console.log(`Item ${itemId}: isExpanded = ${isExpanded}`);
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* Expanded details section - chỉ hiển thị khi thực sự được click */}
                                {isExpanded && (
                                    <View style={styles.expandedDetails}>
                                        <View style={styles.detailItem}>
                                            <Text style={styles.detailItemLabel}>Kỳ hóa đơn:</Text>
                                            <Text style={styles.detailItemValue}>
                                                {typeof selectedInvoice?.period === 'string' 
                                                    ? selectedInvoice.period 
                                                    : selectedInvoice?.period 
                                                        ? `${selectedInvoice.period.month}/${selectedInvoice.period.year}`
                                                        : 'N/A'
                                                }
                                            </Text>
                                        </View>
                                        <View style={styles.detailItem}>
                                            <Text style={styles.detailItemLabel}>Ngày phát hành:</Text>
                                            <Text style={styles.detailItemValue}>
                                                {selectedInvoice?.issueDate ? formatDate(selectedInvoice.issueDate) : 'N/A'}
                                            </Text>
                                        </View>
                                        <View style={styles.detailItem}>
                                            <Text style={styles.detailItemLabel}>Chi tiết đơn giá:</Text>
                                            <Text style={styles.detailItemValue}>
                                                {item.unitPrice ? `${item.unitPrice.toLocaleString('vi-VN')}VND/${item.type === 'variable' ? 'kWh' : 'tháng'}` : 'N/A'}
                                            </Text>
                                        </View>
                                        <View style={styles.detailItem}>
                                            <Text style={styles.detailItemLabel}>Hạn thanh toán:</Text>
                                            <Text style={styles.detailItemValue}>
                                                {selectedInvoice?.dueDate ? formatDate(selectedInvoice.dueDate) : 'N/A'}
                                            </Text>
                                        </View>
                                        <View style={styles.detailItem}>
                                            <Text style={styles.detailItemLabel}>Trạng thái thanh toán:</Text>
                                            <Text style={styles.detailItemValue}>
