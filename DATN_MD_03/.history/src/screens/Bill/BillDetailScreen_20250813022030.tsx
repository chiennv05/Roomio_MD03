...
const BillDetailScreen = () => {
    ...
    const renderInvoiceItems = () => {
        // Debug: kiểm tra selectedInvoice
        console.log('renderInvoiceItems - selectedInvoice:', selectedInvoice);
        console.log('renderInvoiceItems - selectedInvoice?.items:', selectedInvoice?.items);
        
        if (!selectedInvoice || !selectedInvoice.items || selectedInvoice.items.length === 0) {
            console.log('renderInvoiceItems - No items to render');
            return (
        ...
        // Debug: kiểm tra state và items
        console.log('renderInvoiceItems - items:', items);
        console.log('renderInvoiceItems - expandedItems:', Array.from(expandedItems));
        ...
        itemType: item.type
                    ...
                    return (
                        ...
                        >
                            ...
                            {/* Expanded details section - chỉ hiển thị khi được click */}
                            {isExpanded && (
                                <View style={styles.expandedDetails}>
                                    {/* Debug: hiển thị trạng thái expanded */}
                                    <Text style={{color: 'red', fontSize: 10}}>DEBUG: EXPANDED = {isExpanded.toString()}</Text>
                                    <Text style={{color: 'red', fontSize: 10}}>DEBUG: itemId = {itemId}</Text>
                                    <Text style={{color: 'red', fontSize: 10}}>DEBUG: selectedInvoice = {selectedInvoice ? 'EXISTS' : 'NULL'}</Text>
                                    <Text style={{color: 'red', fontSize: 10}}>DEBUG: issueDate = {selectedInvoice?.issueDate || 'NULL'}</Text>
                                    <Text style={{color: 'red', fontSize: 'red', fontSize: 10}}>DEBUG: dueDate = {selectedInvoice?.dueDate || 'NULL'}</Text>
                                    <Text style={{color: 'red', fontSize: 10}}>DEBUG: unitPrice = {item.unitPrice || 'NULL'}</Text>
                                    
                                    <View style={styles.detailItem}>
                                        <Text style={styles.detailItemLabel}>Ngày bắt đầu:</Text>
                                        <Text style={styles.detailItemValue}>
                                            {selectedInvoice?.issueDate ? formatDate(selectedInvoice.issueDate) : 'N/A'}
                                        </Text>
                                    </View>
                                    <View style={styles.detailItem}>
                                        <Text style={styles.detailItemLabel}>Ngày kết thúc:</Text>
                                        <Text style={styles.detailItemValue}>
                                    ...
                                    <View style={styles.detailItem}>
                                        ...
                                        <Text style={styles.detailItemValue}>
                                            {item.unitPrice ? `${item.unitPrice.toLocaleString('vi-VN')}VND/${item.type === 'variable' ? 'kWh' : 'tháng'}` : 'N/A'}
                                        </Text>
                                    </View>
                                </View>
                            )}
    ...
    // Hàm để toggle expanded state của item
    const toggleItemExpanded = (itemId: string) => {
        console.log('toggleItemExpanded called with itemId:', itemId);
        ...
        console.log('New expandedItems:', Array.from(newExpandedItems));
