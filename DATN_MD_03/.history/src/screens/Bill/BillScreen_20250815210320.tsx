...
const BillScreen = () => {
    ...
    useEffect(() => {
        ...
        // Lọc theo khoảng thời gian (ưu tiên createdAt; nếu thiếu dùng period)
        if (startDateFilter || endDateFilter) {
            ...
            allInvoices = allInvoices.filter(inv => {
                ...
                if (s && dateVal < s) {return false;}
                if (e && dateVal > e) {return false;}
                return true;
            });
            console.log('Filtered by date range:', startDateFilter, endDateFilter, '- remaining:', allInvoices.length, '(removed', countBefore - allInvoices.length, ')');
        }
    ...
    }, [invoices, selectedStatus, selectedRoom, selectedTenant, sortOrder, searchText, isLandlord, user?.role, isUserCoTenant]);
