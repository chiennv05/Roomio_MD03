...
const EditInvoiceScreen = () => {
    ...
    // Custom alert hook
    const { showSuccess, showError, showConfirm, visible: alertVisible, alertConfig, hideAlert } = useCustomAlert();
    ...
    const [customItemCategory, setCustomItemCategory] = useState('service');
    const [customItemIsRecurring, setCustomItemIsRecurring] = useState(true);
    const [customItemPreviousReading, setCustomItemPreviousReading] = useState('0');
    const [customItemCurrentReading, setCustomItemCurrentReading] = useState('0');
    ...
    useEffect(() => {
        ...
        if (updateInvoiceError) {
            showError(`Không thể cập nhật hóa đơn: ${updateInvoiceError}`, "Lỗi");
            dispatch(resetUpdateInvoiceState());
        }
    ...
    useEffect(() => {
        ...
        if (addItemError) {
            showError(`Không thể thêm khoản mục tùy chỉnh: ${addItemError}`, "Lỗi");
            dispatch(resetAddItemState());
        }
    ...
    useEffect(() => {
        ...
        if (updateItemsError) {
            showError(`Không thể cập nhật khoản mục hóa đơn: ${updateItemsError}`, "Lỗi");
            dispatch(resetUpdateItemsState());
        }
    ...
    useEffect(() => {
        ...
        if (deleteItemError) {
            showError(`Không thể xóa khoản mục hóa đơn: ${deleteItemError}`, "Lỗi");
            dispatch(resetDeleteItemState());
        }
    ...
    useEffect(() => {
        ...
        if (saveTemplateError) {
            showError(`Không thể lưu mẫu hóa đơn: ${saveTemplateError}`, "Lỗi");
            dispatch(resetSaveTemplateState());
        }
    ...
    const resetCustomItemForm = () => {
        ...
        setCustomItemCategory('service');
        setCustomItemIsRecurring(true);
        setCustomItemPreviousReading('0');
        setCustomItemCurrentReading('0');
    };
    ...
    const handleAddCustomItem = () => {
        ...
        if (!customItemName.trim()) {
            showError("Vui lòng nhập tên khoản mục", "Lỗi");
            return;
        }

        const unitPrice = parseInt(customItemUnitPrice) || 0;
        const quantity = parseInt(customItemQuantity) || 1;

        if (unitPrice <= 0) {
            showError("Đơn giá phải lớn hơn 0", "Lỗi");
            return;
        }
    ...
    const handleBackPress = () => {
        // Directly save changes and go back
    ...
    const handleSaveAndGoBack = () => {
        ...
        if (hasFormErrors()) {
            showError("Vui lòng nhập hợp lệ tất cả các trường trước khi lưu.", "Lỗi");
            return;
        }
    ...
    const handleIssueInvoice = () => {
        ...
        if (hasFormErrors()) {
            showError("Vui lòng nhập hợp lệ tất cả các trường trước khi phát hành hóa đơn.", "Lỗi");
            return;
        }
        ...
        showConfirm(
            ...
            () => {
                        ...
                        saveItems()
                            ...
                            .catch((error) => {
                                setIsLoading(false);
                                showError("Không thể phát hành hóa đơn. Vui lòng thử lại.", "Lỗi");
                            });
                    }
                },
                "Xác nhận phát hành"
    ...
    const handleDeleteItem = (item: InvoiceItem) => {
        if (!token || !selectedInvoice || !item._id) {
            showError("Không thể xóa khoản mục này. Thiếu thông tin cần thiết.", "Lỗi");
            return;
        }
            ...
            () => {
                        ...
                        dispatch(deleteInvoiceItem({
                            ...
                            .catch((error) => {
                                ...
                                showError(`Không thể xóa khoản mục: ${error}`, "Lỗi");
                            });
                    },
                    "Xác nhận xóa"
                );
    ...
    const handleSaveAsTemplate = (templateName: string) => {
        if (!token || !selectedInvoice) {
            showError("Không thể lưu mẫu hóa đơn. Thiếu thông tin cần thiết.", "Lỗi");
            return;
        }

        // Kiểm tra lỗi trước khi lưu mẫu
        if (hasFormErrors()) {
            showError("Vui lòng nhập hợp lệ tất cả các trường trước khi lưu mẫu hóa đơn.", "Lỗi");
            return;
        }
        ...
        saveItems()
            ...
            .catch((error) => {
                ...
                showError(`Không thể lưu hóa đơn hoặc lưu mẫu: ${error}`, "Lỗi");
            });
    };
    ...
    const renderInvoiceInfo = () => {
        ...
        console.log('Contract ID:', selectedInvoice.contractId);
        console.log('Contract Info:', getNestedValue(selectedInvoice.contractId, 'contractInfo'));
        ...
        const getTenantName = () => {
            ...
            if (contractTenantName) {
                console.log('Lấy tên người thuê từ contractId.contractInfo.tenantName:', contractTenantName);
            ...
            if (selectedInvoice.tenantId && typeof selectedInvoice.tenantId === 'object') {
                ...
                if (fullName) {
                    console.log('Lấy tên người thuê từ tenantId.fullName:', fullName);
                    return fullName;
                }
        ...
        const getRoomInfo = () => {
            ...
            if (contractRoomNumber) {
                console.log('Lấy số phòng từ contractId.contractInfo.roomNumber:', contractRoomNumber);
                return contractRoomNumber;
            }
            ...
            if (selectedInvoice.roomId && typeof selectedInvoice.roomId === 'object') {
                ...
                if (roomNumber) {
                    console.log('Lấy số phòng từ roomId.roomNumber:', roomNumber);
                    return roomNumber;
                }
            ...
            <View style={styles.section}>
                ...
                <View style={styles.infoRow}>
                    ...
                    >
                        ...
                        <Image
                            source={Platform.OS === 'ios' ? { uri: Icons.IconCalendar } : { uri: Icons.IconCalendar }}
                    ...
                    <View style={styles.modalContent}>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            ...
                            <View style={styles.modalInfoNote}>
                                <Text style={styles.modalInfoNoteText}>
                                    Lưu ý: Các khoản mục tùy chỉnh có thể được chỉnh sửa hoặc xóa sau khi tạo.
                                </Text>
                                <Text style={styles.modalInfoNoteText}>
                                    • Khoản mục dịch vụ, bảo trì, khác có thể xóa sau khi tạo.
                                </Text>
                            </View>
                            ...
                            <View style={styles.rowInputs}>
                                ...
                                <View style={[styles.inputGroup, { flex: 1 }]}>
                                    ...
                                    <TextInput
                                        ...
                                        onChangeText={setCustomItemUnitPrice}
                                        keyboardType="numeric"
                                        placeholder="0"
                                    />
                                </View>
                            ...
                            <View style={styles.inputGroup}>
                                ...
                                <View style={styles.categoryButtons}>
                                    {['service', 'maintenance', 'other'].map(category => (
                                        <TouchableOpacity
                                            key={category}
                                            style={[
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView style={styles.scrollView}>
                ...
                <View style={styles.buttonGroup}>
                    <TouchableOpacity
                        ...
                        disabled={updateInvoiceLoading || !selectedInvoice}>
            ...
            {/* Custom Alert Modal */}
            <CustomAlertModal
                visible={alertVisible && !!alertConfig}
