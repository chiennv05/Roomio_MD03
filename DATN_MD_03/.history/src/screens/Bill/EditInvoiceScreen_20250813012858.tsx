...
const EditInvoiceScreen = () => {
    ...
    const handleAddCustomItem = () => {
        ...
        if (!customItemName.trim()) {
            ...
            showError("Vui lòng nhập tên khoản mục");
    ...
    const getItemEditability = (item: InvoiceItem): {
        ...
        if (isStandardContractItem(item)) {
            // Các khoản mục từ hợp đồng chỉ có thể chỉnh sửa mô tả và chỉ số đồng hồ (nếu là utility)
            result.isEditable = true;
            result.canEditDescription = true;
            
            // Chỉ cho phép chỉnh sửa chỉ số đồng hồ nếu là utility, nhưng KHÔNG cho phép chỉnh sửa đơn giá
            ...
            <View style={styles.section}>
                          ...
                          {selectedInvoice && selectedInvoice.status === 'draft' && (
                              ...
                              >
                                  ...
                                  <Text style={styles.addItemButtonIcon}>+</Text>
                              </TouchableOpacity>
                          )}
                      </View>
                {selectedInvoice && selectedInvoice.status === 'draft' && (
    ...
    const fetchPreviousInvoiceData = useCallback(async () => {
        ...
        try {
            ...
            if (!contractId) {
                console.log('Không tìm thấy contractId');
                return;
            }
            ...
            if (typeof selectedInvoice.period === 'string') {
                ...
                console.log('Không thể xác định kỳ hóa đơn');
                return;
            }
            ...
            if (response.success && response.data) {
                setPreviousInvoiceData(response.data);
                console.log('Đã lấy được dữ liệu hóa đơn kỳ trước:', response.data);
            } else {
                console.log('Không tìm thấy hóa đơn kỳ trước:', response.message);
            }
        } catch (error) {
...
const styles = StyleSheet.create({
    ...
    addItemButton: {
        backgroundColor: '#BAFD00', // Lime green color như trong ảnh
        paddingHorizontal: 20,
        paddingVertical: 16,
        borderRadius: 8,
        flexDirection: 'row',
        ...
        marginTop: 10, // Thêm khoảng cách với title
    },
    addItemButtonText: {
        color: Colors.black, // Chữ màu đen như trong ảnh
        fontSize: 16,
        fontWeight: '700',
        letterSpacing: 0.5,
    ...
    addItemButtonIcon: {
        color: Colors.black, // Icon màu đen như trong ảnh
        fontSize: 24,
        fontWeight: 'bold',
        marginLeft: 8,
    },
