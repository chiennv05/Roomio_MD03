        ...
        const renderItemDetails = (item: InvoiceItem) => {
            ...
            return (
                <View style={styles.itemDetailsBox}>
                    <Text style={styles.itemDetailsTitle}>Chi tiết {item.name || getCategoryText(item.category)}</Text>
                    {rows.map((row, idx) => (
            ...
            <View style={styles.itemsContainer}>
                <View style={styles.itemsGrid}>
                    {items.map((item, index) => {
                        ...
                        return (
                        <TouchableOpacity key={key} style={styles.itemCard} activeOpacity={0.85} onPress={() => toggleItemExpanded(key)}>
