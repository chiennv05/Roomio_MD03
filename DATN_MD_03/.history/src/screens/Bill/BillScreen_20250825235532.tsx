...
const BillScreen = () => {
    ...
    // State đánh dấu người ở cùng (giữ để không vỡ UI nhưng luôn false theo yêu cầu hiển thị hóa đơn chung cho người thuê)
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            // Yêu cầu mới: người thuê chỉ xem hóa đơn chung => luôn fetchInvoices
            if (isLandlord || user?.role === 'nguoiThue') {
                setIsUserCoTenant(false); // đảm bảo false để UI không đổi tiêu đề
    ...
    useEffect(() => {
        // Luôn fetchInvoices cho cả landlord và tenant theo yêu cầu mới
        if (token) {
            dispatch(fetchInvoices({
                token,
                page: 1,
    ...
    const handleRefresh = useCallback(() => {
        ...
        const refreshData = async () => {
            try {
                ...
                // Người thuê: theo yêu cầu mới chỉ refresh hóa đơn chung
                await dispatch(fetchInvoices({
                    token,
                    page: 1,
                    limit: 10,
