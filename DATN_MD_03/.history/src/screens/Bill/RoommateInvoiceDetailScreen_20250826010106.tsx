...
const RoommateInvoiceDetailScreen = ({ route, navigation }: Props) => {
        ...
        <View style={styles.invoiceDetailsGrid}>
          ...
          {(roommateInvoice.status === 'pending_confirmation' || roommateInvoice.status === 'paid') && !!roommateInvoice.paymentMethod && (
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{String(roommateInvoice.paymentMethod)}</Text>
            </View>
          )}
  ...
  const renderInvoiceItems = () => {
    ...
    const renderItemDetails = (item: any, align: 'left' | 'right') => {
      ...
      // Chỉ hiển thị đọc số nếu có cả hai (giống BillDetailScreen cách thể hiện usage)
    ...
    return (
      <View style={styles.summarySection}>
        <View style={styles.summaryRow}>
          <Text style={styles.summaryTotal}>Tổng tiền</Text>
          <Text style={styles.summaryTotalValue}>{roommateInvoice.totalAmount.toLocaleString('vi-VN')}VND</Text>
        </View>
      </View>
    );
  };
