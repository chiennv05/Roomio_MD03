...
const BillScreen = () => {
    ...
    const handleInvoicePress = (invoice: Invoice) => {
        ...
        if (!invoiceId) {
            CustomAlertModal.show('Lỗi', 'Không thể mở chi tiết hóa đơn này');
            return;
        }
    ...
    const handleEditInvoice = (invoice: Invoice) => {
        ...
        if (!invoiceId) {
            CustomAlertModal.show('Lỗi', 'Không thể chỉnh sửa hóa đơn này');
            return;
        }
    ...
    const handleDeleteInvoice = (invoice: Invoice) => {
        ...
        const invoiceId = invoice._id || invoice.id;
        if (!invoiceId) {
            CustomAlertModal.show('Lỗi', 'Không thể xóa hóa đơn này');
            return;
        }
    ...
    const deleteInvoiceHandler = async (invoiceId: string) => {
        if (!token) {
            CustomAlertModal.show('Lỗi', 'Bạn cần đăng nhập để thực hiện chức năng này');
            return;
        }
        ...
        try {
            ...
            if (response.success) {
                ...
                CustomAlertModal.show('Thành công', response.message || 'Đã xóa hóa đơn thành công');
            ...
            CustomAlertModal.show('Lỗi', error.message || 'Có lỗi xảy ra khi xóa hóa đơn');
        }
    };
