...
export const checkUserIsCoTenant = async (token: string) => {
    try {
        console.log('[DEBUG_COTENANT] Start checking if user is a co-tenant');
        ...
        if ('isError' in userResponse) {
            console.log('[DEBUG_COTENANT] Error getting user profile:', userResponse.message);
        ...
        if (!currentUserId) {
            console.log('[DEBUG_COTENANT] Cannot determine current user ID');
            throw new Error('Không thể xác định ID người dùng hiện tại');
        }

        console.log('[DEBUG_COTENANT] Current user ID:', currentUserId);
        
        // Gọi API mới để lấy danh sách hóa đơn người ở cùng
        console.log('[DEBUG_COTENANT] Calling /billing/roommate/invoices API');
        ...
        if ('isError' in response) {
            console.log('[DEBUG_COTENANT] Error from API:', response.message);
            throw new Error(response.message || 'Có lỗi xảy ra khi kiểm tra trạng thái người ở cùng');
        }

        // Kiểm tra nếu API trả về invoices
        console.log('[DEBUG_COTENANT] API response structure:', {
            success: response.data?.success,
            hasInvoices: !!response.data?.invoices,
            invoicesType: typeof response.data?.invoices,
            responseKeys: Object.keys(response.data || {})
        });
        
        const invoices = response.data?.invoices || [];
        
        console.log('[DEBUG_COTENANT] API returned', invoices.length, 'roommate invoices');
        
        // Nếu có ít nhất một hóa đơn, người dùng là người ở cùng
        const isCoTenant = invoices.length > 0;
        
        // Thu thập danh sách các hợp đồng có người dùng là người ở cùng
        const coTenantContracts: any[] = [];
        if (isCoTenant) {
            console.log('[DEBUG_COTENANT] User is a co-tenant, collecting contract information');
            for (const invoice of invoices) {
                if (invoice.contractId && typeof invoice.contractId === 'object') {
                    // Thêm thông tin hợp đồng vào danh sách nếu chưa có
                    if (!coTenantContracts.some(c => c._id === invoice.contractId._id)) {
        ...
        console.log(`[DEBUG_COTENANT] Final result: User ${currentUserId} isCoTenant:`, isCoTenant, 'Contracts:', coTenantContracts.length);
        ...
        console.error('[DEBUG_COTENANT] Error checking coTenant status:', error.message);
