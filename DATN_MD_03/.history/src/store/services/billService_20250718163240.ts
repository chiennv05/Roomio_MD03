...
export const checkUserIsCoTenant = async (token: string, userId: string) => {
    try {
        // Gọi API lấy danh sách hóa đơn roommate để kiểm tra người dùng có phải là coTenant không
        console.log('Checking if user is coTenant using roommate invoices endpoint', { userId });
        ...
        // Lấy danh sách contracts từ các invoices và kiểm tra coTenants
        const contracts: any[] = [];
        ...
        invoices.forEach(invoice => {
            if (invoice.contractId && typeof invoice.contractId === 'object' && 
                ...
                // Kiểm tra xem userId có trong danh sách coTenants của contract này không
                if (invoice.contractId.contractInfo && 
                    invoice.contractId.contractInfo.coTenants && 
                    Array.isArray(invoice.contractId.contractInfo.coTenants)) {
                    
                    const foundCoTenant = invoice.contractId.contractInfo.coTenants.find(
                        (coTenant: any) => coTenant.userId === userId
                    );
                    
                    if (foundCoTenant) {
                        isCoTenant = true;
                        console.log('Found user in coTenants list:', foundCoTenant);
                    }
                }
            }
        });
        ...
        try {
            ...
            for (const contract of contracts) {
                if (contract.contractInfo && contract.contractInfo.coTenants && 
                    ...
                    const foundCoTenant = contract.contractInfo.coTenants.find(
                        coTenant => coTenant.userId === userId
                    );
                    
                    if (foundCoTenant) {
                        isCoTenant = true;
                        console.log('Found user in coTenants list (fallback):', foundCoTenant);
                        break;
                    }
                }
            }
            
            console.log('Fallback coTenant check result:', { isCoTenant, userId });
