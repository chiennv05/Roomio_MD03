...
// Thêm async thunk để tải hóa đơn người ở cùng
export const fetchRoommateInvoices = createAsyncThunk(
    ...
    async ({
        ...
        try {
            ...
            if (!response.success) {
                return rejectWithValue(response.data || 'Failed to fetch roommate invoices');
            }
            ...
            return rejectWithValue(error.message || 'Có lỗi xảy ra khi tải hóa đơn người ở cùng');
        }
    }
);
    ...
    extraReducers: builder => {
        builder
            ...
            .addCase(fetchInvoices.rejected, (state, action) => {
                state.loading = false;
                state.error = action.payload as string || 'An error occurred while fetching invoices';
            })

            // Handle roommate invoices fetching states
            ...
            .addCase(fetchRoommateInvoices.fulfilled, (state, action) => {
                ...
                if (action.meta.arg.page && action.meta.arg.page > 1) {
                    // If loading more pages, append to existing invoices
                    if (action.payload && action.payload.invoices) {
                        // Ensure all invoices are marked as roommate invoices
                        const roommateInvoices = action.payload.invoices.map((invoice: Invoice) => ({
                            ...invoice,
                            isRoommate: true
                        }));
                        
                        // Check for duplicates before adding
                        const existingIds = new Set(state.invoices.map(inv => inv._id || inv.id));
                        const newInvoices = roommateInvoices.filter(
                            (inv: Invoice) => !existingIds.has(inv._id || inv.id)
                        );
                        
                        state.invoices = [...state.invoices, ...newInvoices];
                    }
                } else {
                    // If loading first page, replace existing invoices
                    state.invoices = action.payload?.invoices?.map((invoice: Invoice) => ({
                        ...invoice,
                        isRoommate: true
                    })) || [];
                }
                ...
                if (action.payload && action.payload.pagination) {
                    state.pagination = {
                        ...
                        totalDocs: action.payload.pagination.totalDocs || 0,
            ...
            .addCase(fetchRoommateInvoices.rejected, (state, action) => {
                state.loading = false;
                state.error = action.payload ? action.payload as string : 'An error occurred while fetching roommate invoices';
            })
            ...
            // Reducers for checkUserCoTenant
            .addCase(checkUserCoTenant.pending, (state) => {
                state.coTenantLoading = true;
                state.coTenantError = null;
            })
            .addCase(checkUserCoTenant.fulfilled, (state, action) => {
                state.coTenantLoading = false;
                if (action.payload) {
                    state.isCoTenant = action.payload.isCoTenant || false;
                    state.coTenantContracts = action.payload.contracts || [];
                } else {
                    state.isCoTenant = false;
                    state.coTenantContracts = [];
                }
            })
            .addCase(checkUserCoTenant.rejected, (state, action) => {
                state.coTenantLoading = false;
                state.coTenantError = action.payload ? action.payload as string : 'Không thể kiểm tra trạng thái người ở cùng';
            });
    },
});
