...
interface BillState {
    ...
    isCoTenantLoading: boolean;
    isCoTenant: boolean;
    isCoTenantError: string | null;
...
const initialState: BillState = {
    ...
    isCoTenantLoading: false,
    isCoTenant: false,
    isCoTenantError: null,
    coTenantContracts: []
};
...
// Thunk để kiểm tra xem người dùng có phải là người ở cùng không
export const checkIsCoTenant = createAsyncThunk(
    'bill/checkIsCoTenant',
    async ({ token }: { token: string }, { rejectWithValue }) => {
        try {
            const response = await checkUserIsCoTenant(token);

            if (!response.success) {
                throw new Error('Không thể kiểm tra trạng thái người ở cùng');
            }

            return {
                isCoTenant: response.isCoTenant,
                contracts: response.contracts || []
            };
        } catch (err: any) {
            return rejectWithValue(err.message || 'Không thể kiểm tra trạng thái người ở cùng');
        }
    }
);
...
const billSlice = createSlice({
    ...
    reducers: {
        ...
        resetCoTenantState: (state) => {
            state.isCoTenantLoading = false;
            state.isCoTenant = false;
            state.isCoTenantError = null;
    ...
    extraReducers: builder => {
        builder
            ...
            // Xử lý checkIsCoTenant
            .addCase(checkIsCoTenant.pending, (state: BillState) => {
                state.isCoTenantLoading = true;
                state.isCoTenantError = null;
            })
            .addCase(checkIsCoTenant.fulfilled, (state: BillState, action: PayloadAction<{isCoTenant: boolean, contracts: any[]}>) => {
                state.isCoTenantLoading = false;
                state.isCoTenant = action.payload.isCoTenant;
                state.coTenantContracts = action.payload.contracts;
                console.log('User isCoTenant status:', state.isCoTenant);
            })
            .addCase(checkIsCoTenant.rejected, (state: BillState, action: PayloadAction<unknown>) => {
                state.isCoTenantLoading = false;
                state.isCoTenantError = action.payload as string;
                state.isCoTenant = false; // Set default value if check fails
            });
    },
});
