...
const EditInvoiceScreen = () => {
    ...
    // State để track invoiceId hiện tại để đảm bảo reset khi thay đổi hóa đơn
    const [currentInvoiceId, setCurrentInvoiceId] = useState<string | null>(null);

    // Load invoice details
    useEffect(() => {
        if (token && invoiceId) {
            // Nếu invoiceId thay đổi hoặc lần đầu mount, reset form state  
            const isInitialMount = !currentInvoiceId;
            const isInvoiceChanged = currentInvoiceId && currentInvoiceId !== invoiceId;
            
            if (isInitialMount || isInvoiceChanged) {
                console.log('Resetting form...', {
                    reason: isInitialMount ? 'Initial mount' : 'Invoice changed',
                    oldId: currentInvoiceId,
                    newId: invoiceId
                });
            ...
            // Cập nhật currentInvoiceId và fetch dữ liệu
        ...
        return () => {
            ...
            setCurrentInvoiceId(null); // Reset current invoice ID
        };
    }, [dispatch, token, invoiceId, currentInvoiceId]);

    // Initialize form with invoice data when available
    useEffect(() => {
        if (selectedInvoice) {
            // Khởi tạo form nếu chưa được khởi tạo hoặc đây là hóa đơn mới
            const shouldInitializeForm = !isFormInitialized || 
                (selectedInvoice._id && selectedInvoice._id === currentInvoiceId);
                
            if (shouldInitializeForm) {
                console.log('Initializing form with invoice data...', selectedInvoice._id);
            ...
            if (selectedInvoice.items && selectedInvoice.items.length > 0) {
                ...
                console.log('Form initialized successfully for invoice:', selectedInvoice._id);
            } else {
                console.log('Skipping form initialization - already initialized or different invoice');
            }

            setTotalAmount(selectedInvoice.totalAmount);
        }
    }, [selectedInvoice, isFormInitialized, currentInvoiceId]);
    ...
    const handleSaveDraft = () => {
        ...
        // Lưu nháp không cần kiểm tra validation nghiêm ngặt
        // Chỉ kiểm tra thông tin cơ bản
        console.log("Saving draft without strict validation...");
