...
const EditInvoiceScreen = () => {
    ...
    // State để theo dõi xem đã khởi tạo form lần đầu chưa
    ...
    useEffect(() => {
        if (selectedInvoice) {
            // Chỉ set initial values lần đầu tiên khi chưa khởi tạo form
            if (!isFormInitialized) {
                ...
                // Đánh dấu đã khởi tạo form
                setIsFormInitialized(true);
            }

            // Luôn cập nhật danh sách items (để có item mới khi thêm/xóa)
            if (selectedInvoice.items && selectedInvoice.items.length > 0) {
                setInvoiceItems([...selectedInvoice.items]);

                // Chỉ khởi tạo itemInputs cho items mới hoặc lần đầu tiên
                const currentItemIds = Object.keys(itemInputs);
                const newItemInputs = { ...itemInputs };

                selectedInvoice.items.forEach((item, index) => {
                    const itemKey = item._id || `item-${index}`;

                    // Chỉ init nếu item này chưa có trong itemInputs (item mới)
                ...
                // Xóa input data của các items đã bị xóa
                const currentInvoiceItemIds = selectedInvoice.items.map((item, idx) => item._id || `item-${idx}`);
                Object.keys(newItemInputs).forEach(itemId => {
                    if (!currentInvoiceItemIds.includes(itemId)) {
    ...
    }, [selectedInvoice, isFormInitialized, itemInputs]);
    ...
    useEffect(() => {
        if (addItemSuccess) {
            ...
            // Reset state thay vì fetch lại (store đã tự động cập nhật selectedInvoice)
            dispatch(resetAddItemState());
        }
    ...
    }, [addItemSuccess, addItemError, dispatch, showSuccess, showError]);
    ...
    useEffect(() => {
        if (updateItemsSuccess) {
            ...
            // Không cần fetch lại - store đã tự động cập nhật
        }
    ...
    }, [updateItemsSuccess, updateItemsError, dispatch, showSuccess, showError]);
    ...
    }, [deleteItemSuccess, deleteItemError, dispatch, showSuccess, showError]);
        ...
        return (
            <AddCustomItemModal
                ...
                onSave={(data) => {
                    ...
                    // Modal sẽ được đóng trong useEffect khi addItemSuccess = true
                }}
            />
        );
    };
    ...
    const fetchPreviousInvoiceData = useCallback(async () => {
        console.log('fetchPreviousInvoiceData called');
        if (!token || !selectedInvoice || !selectedInvoice.contractId || !selectedInvoice.period) {
            console.log('Missing required data for fetchPreviousInvoiceData');
            return;
        }

        console.log('Setting loadingPreviousInvoice to true');
        ...
        try {
            ...
            console.log('getPreviousInvoice response:', response);
            ...
            console.log('Setting loadingPreviousInvoice to false');
    ...
    console.log('Render state:', {
        ...
        selectedInvoiceStatus: selectedInvoice?.status
    });
